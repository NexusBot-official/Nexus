...
module.exports = {
  ...
  async execute(channel, client) {
    // Ignore non-guild channels (DMs/others) which don't have .guild
    if (!channel || !channel.guild) {
      logger.warn(
        "channelDelete",
        "channel or channel.guild undefined, skipping"
      );
      return;
    }
      ...
      try {
        ...
        if (entry && entry.executor) {
          ...
          await client.advancedAntiNuke.monitorAction(
            ...
            "channelDelete",
            entry.executor.id,
            { channelId: channel.id, channelName: channel.name || "<unknown>" }
          );
        }
      } catch (error) {
    ...
    await EnhancedLogging.log(channel.guild.id, "channel_delete", "server", {
      ...
      details: `Channel deleted: ${channel.name || "<unknown>"}`,
      metadata: {
    ...
    await client.db.addEnhancedLog(
      ...
      null,
      "channel_deleted",
      `Channel ${channel.name || "<unknown>"} was deleted`,
      {
        channelId: channel.id || "<unknown>",
        channelName: channel.name || "<unknown>",
      },
    ...
    if (config && config.mod_log_channel) {
      ...
      if (logChannel) {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `**${channel.name || "<unknown>"}** channel was deleted`
          )
          .addFields(
            {
              name: "Channel Name",
