...
module.exports = {
  ...
  async execute(member, client) {
    ...
    // Revoke API key if user left the support server
    const supportServerId = process.env.SUPPORT_SERVER_ID;
    if (supportServerId && member.guild.id === supportServerId) {
      try {
        const apiKey = await db.getUserAPIKey(member.user.id);
        if (apiKey && apiKey.is_active === 1) {
          ...
          logger.warn(
            "API Key",
            `🚫 API key auto-revoked | User: ${member.user.id} (${member.user.tag}) left support server`
          );
        }
      } catch (error) {
        // Silently fail - don't break the leave event
        console.error("Failed to revoke API key on leave:", error);
      }
    }
  },
};
