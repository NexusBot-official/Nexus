...
async function doBackgroundSync() {
  ...
  try {
    ...
    console.log(
      `[Service Worker] Syncing ${pendingActions.length} pending actions`
    );
    ...
    for (const action of pendingActions) {
      try {
        ...
        if (response.ok) {
          ...
          console.warn(
            `[Service Worker] Failed to sync action: ${action.id}, status: ${response.status}`
          );
        }
      } catch (error) {
        console.error(
          `[Service Worker] Error syncing action ${action.id}:`,
          error
        );
      }
    }
