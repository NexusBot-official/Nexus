  ...
  async execute(interaction) {
    ...
    if (subcommand === "create") {
      ...
      const commands = await new Promise((resolve, reject) => {
        // Try new schema first (response_content)
        db.db.all(
          "SELECT command_name, COALESCE(response_content, response) as response FROM custom_commands WHERE guild_id = ?",
          [interaction.guild.id],
          (err, rows) => {
            if (err) {
              // If response_content doesn't exist, try old schema
              if (
                err.message &&
                err.message.includes("no such column: response_content")
              ) {
                db.db.all(
                  ...
                  (err2, rows2) => {
                    if (err2) {
                      // If response also doesn't exist, try just command_name
                      db.db.all(
                        "SELECT command_name FROM custom_commands WHERE guild_id = ?",
                        [interaction.guild.id],
                        (err3, rows3) => {
                          if (err3) {
                            reject(err3);
                ...
                err.message &&
                err.message.includes("no such column: response")
              ) {
                // Only response_content exists
                db.db.all(
                  "SELECT command_name, response_content as response FROM custom_commands WHERE guild_id = ?",
                  [interaction.guild.id],
                  (err2, rows2) => {
                    if (err2) {
                      reject(err2);
