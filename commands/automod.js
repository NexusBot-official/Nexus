...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (subcommand === "spam") {
      ...
      const updates = {};
      if (maxMessages) {
        updates.spam_max_messages = maxMessages;
      }
      if (timeWindow) {
        updates.spam_time_window = timeWindow * 1000;
      }
      if (action) {
        updates.spam_action = action;
      }
        ...
        .addFields(
          {
            ...
            value: config.spam_enabled
              ? `✅ ${config.spam_max_messages || "N/A"} msgs / ${
                  config.spam_time_window
                    ? config.spam_time_window / 1000
                    : "N/A"
                }s → ${config.spam_action || "delete"}`
              : "❌ Disabled",
            inline: false,
          },
          {
          ...
          },
          {
            ...
            value: config.caps_enabled
              ? `✅ ${config.caps_threshold || "N/A"}% → ${config.caps_action || "delete"}`
              : "❌ Disabled",
            inline: true,
          },
          {
            ...
            value: config.emoji_spam_enabled
              ? `✅ Max: ${config.emoji_max_count || "N/A"}`
              : "❌ Disabled",
            inline: true,
          },
          {
            ...
            value: config.mention_spam_enabled
              ? `✅ Max: ${config.mention_max_count || "N/A"}`
              : "❌ Disabled",
            inline: true,
          },
          {
            name: "Invite Blocking",
            value: config.block_invites
              ? `✅ Enabled → ${config.link_action || "delete"}`
              : "❌ Disabled",
            inline: true,
          }
        )
