...
class AdvancedAntiRaid {
  ...
  static detectionAlgorithms = {
    ...
    patternBased: (joins) => {
      ...
      if (variance < 43200000 && joins.length >= 7) {
        return true;
      } // Less than 12 hours variance, need 7+ joins
      ...
      if (commonPatterns.length > 2 && joins.length >= 7) {
        return true;
      } // Need more patterns and joins
    ...
    behavioral: (joins) => {
      ...
      if (noAvatarCount / joins.length > 0.9) {
        return true;
      } // Very high threshold (was 0.8)
    ...
    networkBased: async (joins, guildId) => {
      ...
      if (timeClusters.length > 0 && timeClusters[0].length >= 3) {
        return true;
      }
    ...
    temporalPattern: (joins) => {
      ...
      if (variance < 1000000 && intervals.length >= 5) {
        return true;
      } // < 1 second variance
  ...
  static getServerSizeMultiplier(memberCount) {
    if (memberCount < 100) {
      return 1.0;
    } // Small: 5 joins/10s (STRICT)
    if (memberCount < 500) {
      return 1.6;
    } // Medium: 8 joins/10s (BALANCED)
    if (memberCount < 2000) {
      return 3.0;
    } // Large: 15 joins/10s (RELAXED)
  ...
  static async detectRaid(guild, member) {
    ...
    if (isWhitelisted) {
      return false;
    } // Skip detection for whitelisted users
      ...
      db.db.get(
        ...
        (err, row) => {
          if (err) {
            ...
            resolve(row ? JSON.parse(row.data) : { joins: [] });
          }
        }
      );
    });
  ...
  static async handleRaid(
    ...
    if (isLargeServer && finalMultiplier > 1.5) {
      ...
      const highlySuspicious = suspiciousJoins.filter((join) => {
        ...
        if (Date.now() - join.createdTimestamp < 86400000) {
          suspicionScore += 2;
        } // Less than 1 day old
        if (Date.now() - join.createdTimestamp < 3600000) {
          suspicionScore += 1;
        ...
        if (parseInt(join.discriminator) < 1000) {
          suspicionScore += 1;
        } // Default discriminator

        // If raid is confirmed and we have many joins, lower threshold
        if (suspiciousJoins.length >= 10) {
          suspicionScore += 1;
        } // Bonus point for confirmed raid
    ...
    const logChannel = guild.channels.cache.find(
      (ch) =>
        (ch.name.includes("log") || ch.name.includes("mod")) && ch.isTextBased()
    );
