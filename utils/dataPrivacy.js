...
class DataPrivacy {
  ...
  static async exportUserData(userId, guildId = null) {
    try {
      ...
      try {
        ...
        logger.error(
          `Error fetching scheduled_actions for user export:`,
          error
        );
  ...
  static async deleteUserData(userId, guildId = null) {
    try {
      ...
      try {
        const scheduledWhereClause = guildId
          ? "created_by = ? AND guild_id = ?"
          : "created_by = ?";
        const scheduledParams = guildId ? [userId, guildId] : [userId];

        await new Promise((resolve, reject) => {
          db.db.run(
            `DELETE FROM scheduled_actions WHERE ${scheduledWhereClause}`,
            scheduledParams,
            function (err) {
              if (err) {
                reject(err);
        ...
        logger.error(`Error deleting from scheduled_actions:`, error);
        summary.errors.push({
          table: "scheduled_actions",
          error: error.message,
        });
