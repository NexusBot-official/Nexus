...
class AdvancedAutomod {
  ...
  isIgnored(message, config) {
    ...
    if (cached.channels.includes(message.channel.id)) {
      return true;
    }
  ...
  async checkMessage(message) {
    ...
    if (this.isIgnored(message, config)) {
      return null;
    }
    ...
    const checks = await Promise.all([
      ...
      // Check invites separately - should work even if link_scanning_enabled is false
      config.block_invites ? this.checkInvites(message, config) : null,
  ...
  async checkInvites(message, config) {
    ...
    if (config.invite_whitelist) {
      ...
      if (!isWhitelisted) {
        return {
          type: "invite",
          action: config.link_action || "delete",
  ...
  async checkTestUrl(message, config, testUrl) {
    // Check if message contains the test URL (with or without protocol)
    const content = message.content.toLowerCase();

    // Check if the domain appears anywhere in the message (simple check first)
    if (content.includes(testUrl)) {
      return {
        type: "malicious_link",
        action: config.link_action || "delete",
        reason: "Test URL flagged for AutoMod testing",
        url: testUrl,
      };
    }

    // Also check for full URLs with protocol
    const urlRegex =
      /(https?:\/\/)?([^\s]+\.)?test-automod-flag\.com(\/[^\s]*)?/i;
  ...
  async executeAction(message, violation, config) {
    try {
      ...
      // Delete message if needed (check permissions first)
      if (["delete", "timeout", "kick", "ban"].includes(violation.action)) {
        if (
          ...
          await message.delete().catch(() => {
            // Silently fail - permission issues are expected in some cases
          });
        }
      }
      ...
      switch (violation.action) {
        ...
        case "timeout":
          if (message.guild.members.me.permissions.has("ModerateMembers")) {
            await member
              .timeout(5 * 60 * 1000, `Automod: ${violation.reason}`)
              .catch(() => {});
            await this.sendWarning(message, violation).catch(() => {});
          }
          break;
        ...
        case "ban":
          if (message.guild.members.me.permissions.has("BanMembers")) {
            await member
              .ban({ reason: `Automod: ${violation.reason}` })
              .catch(() => {});
          }
          break;
      }
      ...
      if (config.automod_log_channel) {
        await this.logToChannel(message, violation, config).catch(() => {});
      }
    } catch (error) {
